        ...
        try {
            ...
            window.firebaseOnSnapshot(docRef, (doc) => {
                if (doc.exists() && !this.isSyncing) {
                    ...
                    const localLastSync = localStorage.getItem(`lastSync_${this.userId}`);
                    
                    if (cloudData.lastSync !== localLastSync) {
                        console.log('🔄 クラウドから更新を検出');
                        this.manager.data = cloudData.data;
                        this.manager.saveData();
                        if (window.ui) {
                            window.ui.renderCategories();
                            window.ui.updateStats();
                        }
                        localStorage.setItem(`lastSync_${this.userId}`, cloudData.lastSync);
                    }
                }
            });

            console.log('✅ 自動同期を有効化しました');
        } catch (error) {
...
class BookmarkManager {
    constructor() {
        ...
        this.data = { categories: [] }; // 空のデータ構造で初期化
    }

    async init() {
        // クラウド同期マネージャーを初期化
        this.syncManager = new CloudSyncManager(this);

        // クラウドからデータを取得
        const cloudData = await this.syncManager.downloadFromCloud();
        
        if (cloudData) {
            console.log('クラウドデータを使用します');
            this.data = cloudData;
        } else {
            // ローカルデータまたはデフォルトデータを使用
        ...
        // 自動同期を有効化
        await this.syncManager.enableAutoSync();
    ...
    loadData() {
        try {
            ...
            const saved = localStorage.getItem(`bookmarkData_${userId}`);
    ...
    saveData() {
        try {
            ...
            // 自動でクラウドに同期
            if (this.syncManager) {
                this.syncManager.updateSyncStatus('💾 ローカル保存', 'local');
            }
        } catch (error) {
    ...
    setupEventListeners() {
        ...
        if (syncBtn) {
            syncBtn.addEventListener('click', async () => {
                ...
                if (this.manager.syncManager) {
                    await this.manager.syncManager.uploadToCloud();
                }
            });
...
async function showMainApp(authManager) {
    ...
    // BookmarkManager を初期化（非同期）
    const bookmarkManager = new BookmarkManager();
    await bookmarkManager.init();
    
    // データが正しく初期化されたか確認
    console.log('BookmarkManager データ:', bookmarkManager.data);
