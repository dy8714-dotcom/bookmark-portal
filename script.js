...
class UIManager {
        ...
        card.innerHTML = `
            <div class="category-header" style="background: ${category.color};">
                ...
                <div class="category-actions">
                    <button class="icon-btn" onclick="ui.openCategoryModal('${category.id}')" title="編集">✏️</button>
                    <button class="icon-btn" onclick="ui.deleteCategory('${category.id}')" title="削除">🗑️</button>
                </div>
            </div>
    ...
    renderBookmarks(category) {
        ...
        return category.bookmarks.map(bookmark => `
            <div class="bookmark-item" onclick="ui.openBookmark('${this.escapeHtml(bookmark.url)}')">
